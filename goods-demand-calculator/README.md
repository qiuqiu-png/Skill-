...
自动化计算珠宝门店的补货需求量，支持DIY、戒指、普通商品三种差异化需求规则。

## 功能特性

✅ **智能筛选**：自动筛选符合条件的商品（足金/足金（新）、21天周期、有补单数）
✅ **门店识别**：自动识别所有门店及其对应的库存、销售、在途、需求列
✅ **三类规则**：DIY商品、非DIY戒指、非DIY非戒指使用不同的需求计算公式
...
| 条件 | 补单数量 | 销售量 | 需求值 |
|------|----------|--------|--------|
| 库存+在途-销售 < 0 | > 500 | - | \|库存+在途-销售\| × 5 |
| 库存+在途-销售 < 0 | ≤ 500 | - | \|库存+在途-销售\| × 2 |
| 库存+在途-销售 = 0 | - | - | 1 |
| 库存+在途-销售 > 0 | - | > 1 | 1 |
| 库存+在途-销售 > 0 | - | ≤ 1 | 不填充 |

### 非DIY戒指（二级分类≠DIY，小类=戒指）
...
### v3.0 (2024-12-24)
- ✅ 重构需求计算逻辑：支持三类商品（DIY、非DIY戒指、非DIY非戒指）
- ✅ 新增C列小类判断（戒指识别）
- ✅ 非DIY戒指规则：calc>0时库存+在途2~4且销售>1填1
- ✅ 非DIY非戒指规则：calc=0时库存+在途≤1且销售=1填1
- ✅ 更新文档，新增三类规则对比表格

### v2.2 (2024-12-24)
- ✅ 修正DIY商品判断条件：使用CI列补单数量代替CJ列补单重量
- ✅ 更新所有文档说明
...
**Q: 为什么要区分DIY、戒指、普通商品三类？**
A: 不同商品类型的补货策略不同。DIY商品有特殊倍率（补单数量>500时×5，≤500时×2），戒指在库存适中时（2~4件）也需要补货，普通商品则更保守。
